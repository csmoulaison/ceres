...
void game_update(Game* game, bool up, bool down, bool left, bool right, float dt) {
	...
	f32 movement_friction = 4.0f;
	game->ship_velocity[0] = apply_friction(game->ship_velocity[0], movement_friction, dt);
	game->ship_velocity[1] = apply_friction(game->ship_velocity[1], movement_friction, dt);
